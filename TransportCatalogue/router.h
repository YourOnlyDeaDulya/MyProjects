...
public:
    ...
    Router(const Graph& graph, RoutesInternalData&& rid);
    Router() = default;
...
public:
    using RID = RouteInternalData;

    const RoutesInternalData& GetRID() const
    {
        return routes_internal_data_;
    }
    RoutesInternalData& GetRID()
    {
        return routes_internal_data_;
    }
};
...
template <typename Weight>
Router<Weight>::Router(const Graph& graph, RoutesInternalData&& rid) : graph_(graph), routes_internal_data_(rid)
{
