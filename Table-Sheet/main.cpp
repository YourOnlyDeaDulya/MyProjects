...
void TestSetGetValidFormulaAndRefs() {
    auto sheet = CreateSheet();

    sheet->SetCell("A9"_pos, "2");
    sheet->SetCell("A1"_pos, "=2");

    auto checkCellDouble = [&](Position pos, std::string text, double res) {
        sheet->SetCell(pos, text);
        CellInterface* cell = sheet->GetCell(pos);
        ASSERT(cell != nullptr);
        ASSERT_EQUAL(cell->GetText(), text);
        try {
            std::get<double>(cell->GetValue());
        }
        catch (...) {
            std::cout << "Error: wrong index for variant... " << std::endl;
        }
        ASSERT_EQUAL(std::get<double>(cell->GetValue()), res);
        };

    checkCellDouble("A17"_pos, "=A1+5+A1+A9", 11);
    checkCellDouble("A14"_pos, "=A1+5", 7);
    checkCellDouble("A13"_pos, "=2+5", 7);
    checkCellDouble("A10"_pos, "=A9+5", 7);
    ...
    checkCell("A10"_pos, "2+5");

    checkCellDouble("A10"_pos, "=A9+5", 7);

    checkCellDouble("A10"_pos, "=A17+5", 16);

    checkCellDouble("A10"_pos, "5", 5);

    checkCell("A10"_pos, "text");
