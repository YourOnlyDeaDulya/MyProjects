...
public:
    ...
    double Evaluate(const SheetInterface& sheet) const override {
        try
        {
            ...
            //Здесь любые исключения, которые будут выброшены, будут различными
            //вариантами FormulaError (Либо Div0, либо исключения, которые выбросят методы операндов
        {
            throw fe;
        }
    }
...
public:
    ...
    double Evaluate(const SheetInterface& sheet) const override {
        try {
            ...
            //Тут так же: исключения бросают только методы операндов
        {
            throw fe;
        }
    }
...
public:
    ...
    double Evaluate(const SheetInterface& sheet) const override {
        try
        {
            auto formula_cell = sheet.GetCell(*cell_);
            ...
            {
                const auto& val = formula_cell->GetValue();
                switch (val.index())
                {
                case 0:
                {
                    std::size_t pos;
                    const std::string& word = std::get<0>(val);
                    auto double_val = std::stod(word, &pos);
                    if (pos < word.size())
                    {
        ...
        catch ([[maybe_unused]] const std::invalid_argument& i)
        {
            throw FormulaError(FormulaError::Category::Value);
        }
...
double FormulaAST::Execute(const SheetInterface& sheet) const {
    ...
    //Тут ловим только FormulaError, потому что бросить может только Evaluate()
    catch(const FormulaError& fe)
    {
        throw fe;
    }
